...
A Model Context Protocol (MCP) server that exposes NSO network automation
capabilities through LlamaIndex function tools.

This server allows AI clients to interact with NSO devices through
standardized MCP protocol calls.
...
class NSOMCPServer:
    """MCP Server for NSO network automation."""
    
    def __init__(self):
        self.server = Server("nso-mcp-server")
        self.nso_tools = None
        self.setup_handlers()
        ...
        async def handle_call_tool(name: str, arguments: Dict[str, Any]) -> CallToolResult:
            ...
            try:
                ...
                if name == "show_all_devices":
                    result = self.nso_tools.show_all_devices()
                elif name == "get_router_version":
                    result = self.nso_tools.get_router_version(arguments["router_name"])
                elif name == "get_router_clock":
                    result = self.nso_tools.get_router_clock(arguments["router_name"])
                elif name == "show_router_interfaces":
                    result = self.nso_tools.show_router_interfaces(arguments["router_name"])
                elif name == "get_router_bgp_summary":
                    result = self.nso_tools.get_router_bgp_summary(arguments["router_name"])
                elif name == "get_router_isis_neighbors":
                    result = self.nso_tools.get_router_isis_neighbors(arguments["router_name"])
                elif name == "get_router_ospf_neigh":
                    result = self.nso_tools.get_router_ospf_neigh(arguments["router_name"])
                elif name == "check_cpu":
                    result = self.nso_tools.check_cpu(arguments["router_name"])
                elif name == "check_memory":
                    result = self.nso_tools.check_memory(arguments["router_name"])
                elif name == "ping_router":
                    result = self.nso_tools.ping_router(arguments["router_name"], arguments["ip_address"])
                elif name == "traceroute_router":
                    result = self.nso_tools.traceroute_router(arguments["router_name"], arguments["ip_address"])
                elif name == "lldp_nei":
                    result = self.nso_tools.lldp_nei(arguments["router_name"])
                elif name == "check_alarm":
                    result = self.nso_tools.check_alarm(arguments["router_name"])
                elif name == "iterate":
                    result = self.nso_tools.iterate(arguments["cmd"])
                else:
    ...
    async def initialize(self):
        """Initialize the MCP server with NSO connection."""
        logger.info("🚀 Initializing NSO MCP Server...")
        ...
        self.nso_tools = NSOMCPTools(root)
    ...
    async def run(self):
        ...
        if not await self.initialize():
            logger.error("❌ Failed to initialize NSO MCP Server")
            return
