#!/usr/bin/env python3
"""
NSO Multi-Agent Notebook Test Script
===================================

This script runs the key cells from the NSO_python_multi-agend.ipynb notebook
to test the multi-agent functionality and troubleshoot any issues.
"""
...
def test_nso_connection():
    """Test NSO connection and create tools"""
    ...
    try:
        ...
        m = maapi.Maapi()
        m.start_user_session('admin', 'multi_agent_context')
        ...
        if devices:
            ...
            device_info = root.devices.device[test_device]
            print(f"🧪 Testing device info for {test_device}:")
            print(f"   - Address: {device_info.address}")
            print(f"   - Port: {device_info.port}")
            print(f"   - Authgroup: {device_info.authgroup}")
    ...
    try:
        ...
        def show_all_devices() -> str:
            ...
            try:
                ...
                for device in devices:
                    device_info = root.devices.device[device]
                    result += f"- {device}: {device_info.address}:{device_info.port}\n"
        ...
        def get_device_info(device_name: str) -> str:
            ...
            try:
                ...
                result += f"- Port: {device.port}\n"
                result += f"- Authgroup: {device.authgroup}\n"
        ...
        def get_device_version(device_name: str) -> str:
            ...
            try:
                device = root.devices.device[device_name]
                result = device.live_status.cisco_ios_xr_stats__exec.any.get("show version")
        ...
        print(f"❌ Error creating NSO tools: {e}")
        return {}
...
def main():
    ...
    print("🚀 NSO Multi-Agent Notebook Test")
    print("=" * 50)
    ...
    # Test NSO connection
    devices = test_nso_connection()
    if not devices:
        print("❌ NSO connection failed")
        return False
    
    # Create NSO tools
    nso_tools = create_nso_tools()
    if not nso_tools:
