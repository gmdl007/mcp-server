#!/usr/bin/env python3
"""
Pure LlamaIndex NSO MCP Server
Uses LlamaIndex's FunctionTool and MCP integration
"""
...
def get_devices() -> str:
    ...
    try:
        ...
        return ", ".join(names) if names else "No devices found"
...
# Create LlamaIndex FunctionTools with explicit schemas
llama_tools = [
    FunctionTool(
        fn=get_devices,
...
async def list_tools() -> ListToolsResult:
    ...
    for llama_tool in llama_tools:
        # Debug logging
        logger.debug(f"Tool: {llama_tool.metadata.name}")
        logger.debug(f"fn_schema type: {type(llama_tool.metadata.fn_schema)}")
        logger.debug(f"fn_schema value: {llama_tool.metadata.fn_schema}")
        
        # Convert LlamaIndex FunctionTool to MCP Tool
        # Use the fn_schema directly since we defined it as a dict
        schema = llama_tool.metadata.fn_schema or {"type": "object", "properties": {}, "required": []}
